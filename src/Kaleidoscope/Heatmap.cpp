...
const cRGB Heatmap::log_colors_[][3] = {{0, 128, 0}, {0, 128, 128}, {0, 223, 255},
    {0, 191, 255}, {0, 159, 255}, {0, 127, 255},
    {0, 95, 255}, {0, 63, 255}, {0, 0, 255}};
...
Key Heatmap::eventHook(Key mapped_key, byte row, byte col, uint8_t key_state) {
  ...
  total_keys_++;
  if (heatmap_[row][col] < Heatmap::key_saturation) {
      heatmap_[row][col]++;
  }
...
Heatmap::loopHook(bool is_post_clear) {
  if (total_keys_ > total_saturation) {
    if (Heatmap::clear_on_saturation) {
      clearStats();
    } else {
      shiftStats();
    }
